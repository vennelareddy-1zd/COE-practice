...
if model is not None and df is not None:
    ...
    if st.button("🚀 Predict Cluster", use_container_width=True, key="predict_btn"):
        ...
        scaler.fit(df_encoded[['Gender', 'Age', 'Annual Income (k$)', 'Spending Score (1-100)']])
        ...
        with col2_info:
            ...
            clustered_data = pd.read_csv("clusters_data.csv")
        ...
        with viz_col1:
            ...
            clustered_df = pd.read_csv("clusters_data.csv")
           
            # Inverse transform to original scale for visualization
            # Add Gender column (set to 0 for Male as default for visualization)
            clustered_with_gender = clustered_df.copy()
            clustered_with_gender.insert(0, 'Gender', 0)
            original_scaled = clustered_with_gender[['Gender', 'Age', 'Annual Income (k$)', 'Spending Score (1-100)']].values
